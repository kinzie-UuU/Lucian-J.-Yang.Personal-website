// ── Hero Bamboo v2 — 紧凑版（中式插花参考）────────────────────────────────────
// 竹茎细、节段少、叶片短、花瓶小（y=-0.90 到 -0.28）
// 相机：FOV 28, z=14.0
...
const initHeroBamboo = (canvas) => {
  ...
  const addStalk = (bx, by, lean, segN, r0) => {
    for (let s = 0; s < segN; s++) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        positions.push(cx + Math.cos(theta) * (r + 0.005), cy + h * 0.5, Math.sin(theta) * (r + 0.005));
  ...
  const addLeaf = (ox, oy, dx, dy, len, spread) => {
    for (let i = 0; i < 18; i++) {
      ...
      const nx = -dy, ny = dx;
      positions.push(ox + dx * len * t + nx * sp, oy + dy * len * t + ny * sp, (Math.random() - 0.5) * 0.02);
  ...
  [
    [ 0.00, -0.28,  0.0, 5, 0.009],
    [-0.06, -0.28, -0.6, 4, 0.008],
    [ 0.06, -0.28,  0.5, 4, 0.008],
    [-0.11, -0.28, -1.1, 3, 0.007],
    [ 0.11, -0.28,  1.0, 3, 0.007],
  ].forEach(([bx, by, lean, segN, r0]) => addStalk(bx, by, lean, segN, r0));

  const L = 0.30, S = 0.055;
  [
    [ 0.00,  0.30, -0.82,  0.57, L, S],
    [ 0.00,  0.46,  0.78,  0.63, L, S],
    [ 0.00,  0.58, -0.60,  0.80, L*0.8, S],
    [ 0.00,  0.68,  0.50,  0.87, L*0.7, S*0.8],
    [-0.08,  0.20, -0.90,  0.44, L*0.9, S],
    [-0.08,  0.36, -0.70,  0.71, L*0.85, S],
    [-0.12,  0.50, -0.50,  0.87, L*0.7, S*0.8],
    [ 0.08,  0.18,  0.88,  0.47, L*0.9, S],
    [ 0.08,  0.34,  0.72,  0.69, L*0.85, S],
    [ 0.12,  0.48,  0.55,  0.84, L*0.7, S*0.8],
    [-0.15,  0.10, -0.95,  0.31, L*0.8, S*0.9],
    [-0.15,  0.26, -0.80,  0.60, L*0.75, S*0.8],
    [ 0.15,  0.08,  0.94,  0.34, L*0.8, S*0.9],
    [ 0.15,  0.24,  0.82,  0.57, L*0.75, S*0.8],
    [-0.03,  0.76, -0.45,  0.89, L*0.6, S*0.7],
    [ 0.03,  0.76,  0.40,  0.92, L*0.6, S*0.7],
  ].forEach(([ox, oy, dx, dy, len, spread]) => addLeaf(ox, oy, dx, dy, len, spread));

  const vaseProfile = [
    [-0.90, 0.11, 0.05], [-0.80, 0.13, 0.06], [-0.70, 0.14, 0.065],
    [-0.60, 0.12, 0.055], [-0.50, 0.09, 0.045], [-0.38, 0.10, 0.05],
    [-0.28, 0.12, 0.06],
  ];
